...
void displayAct(activity *&info, int &act_count)
{
  ...
  if (info->user_id && info->title)
  {
    cout << "UserID: " << info->user_id << endl
         << "Activity Number [" << act_count++ << ']' << endl
    ...
    for (int day = 0; day < DAYS_IN_MONTHS[mon]; day++, temp_total_days++)
    {
      for (int hr = 0; hr < 24; hr++)
        for (int act = 0; act < getActs(calendar[mon][day][hr]); act++)
        {
          ...
          if (!strcmp(userID,
                      calendar[mon][day][hr][act]->user_id))
          { // found activity
...
########################################################
  ...
  while (cin >> inp)
  {
    ...
    if (s == '0')
    {
      char *user;
      user = getID();
      int st_mon = getMonth(), end_month = getMonth(), start_day = getDay(),
          end_day = getDay();
      lstAct(user, st_mon, end_month, start_day, end_day);
    }
    ...
    else if (s == '1')
    {
      ...
      lstImpAct(user, st_mon, end_month, start_day, end_month);
    }
    ...
    else if (s == '4')
    {
      ...
      int st = getMonth(), en = getMonth(), sd = getDay(), ed = getDay();

      lstFreeSlots(userIDS, num, st, en, sd, ed);
